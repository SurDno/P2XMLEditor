	...
	public void ProcessFile(string filePath, List<RawEventData> raws) {
		...
		while (xr.Read()) {
			...
			var raw = new RawEventData {
				...
				Manual = xr.Name == "Manual" ? xr.GetBoolValueAndAdvance() : true,
				EventRaisingType = xr.Name == "EventRaisingType" ? xr.GetStringValueAndAdvance().Deserialize<EventRaisingType>() : EventRaisingType.Condition,
				ConditionId = xr.Name == "Condition" ? xr.GetULongValueAndAdvance() : null,
				ChangeTo = xr.Name == "ChangeTo" ? xr.GetBoolValueAndAdvance() : true,
				Repeated = xr.Name == "Repeated" ? xr.GetBoolValueAndAdvance() : true,
