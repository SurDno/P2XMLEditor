...
public static class XElementExtensions {
	...
	public static float ParseDemoFloat(this XElement element) => float.Parse(element.Value.Replace(',', '.'));
	...
	public static Vector3 ParseDemoVector3(this XElement element) {
		var parts = element.Value.Split(' ', StringSplitOptions.RemoveEmptyEntries);
		return new Vector3(
	...
	public static List<string> ParseDemoList(XElement element, string name) {
	...
	public static List<ulong> ParseDemoListAsUlong(XElement element, string name) {
		var listElement = element.Element(name + ".List");
		if (listElement == null || listElement.IsEmpty) return [];
		return listElement.Elements().Select(x => ulong.Parse(x.Value)).ToList();
	}

	public static Dictionary<string, string> ParseDemoDict(XElement element, string name) {
		var dict = new Dictionary<string, string>();
		var dictElement = element.Element(name + ".Dict");
		if (dictElement != null) {
			foreach (var item in dictElement.Elements()) {
	...
	public static (string, ulong)[] ParseDemoDictAsUlong(XElement element, string name) {
		var dictElement = element.Element(name + ".Dict");
		if (dictElement == null) return [];
		var elements = dictElement.Elements().ToList();
		var result = new (string, ulong)[elements.Count];
		for (var i = 0; i < elements.Count; i++) {
			result[i] = (elements[i].Name.LocalName, ulong.Parse(elements[i].Value));
		}
		return result;
	}
